...
def RefPull89(image):
  ...
  d1 = d.eq(1).rename('dswe1').selfMask().updateMask(f.eq(0)).updateMask(r.eq(1))
  #calculate hillshade
  ...
  dswe3 = d.eq(3).rename('dswe3').selfMask().updateMask(f.eq(0)).updateMask(r.eq(1))
  ...
  .combine(ee.Reducer.count().unweighted().forEachBand(pixOut.select(['Aerosol','Blue', 'Green', 'Red', 'Nir', 'Swir1', 'Swir2','SurfaceTemp'])), outputPrefix = 'pCount_', sharedInputs = False)
