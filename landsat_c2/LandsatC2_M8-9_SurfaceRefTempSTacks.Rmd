---
title: "Pull Landsat Collection 2 Surface Reflectance andTemperature Stacks for Missions 8 and 9"
author: "B Steele"
format: html
jupyter: python3
---
...
# existing band names
bn89 = ['SR_B1', 'SR_B2', 'SR_B3', 'SR_B4', 'SR_B5', 'SR_B6', 'SR_B7', 'QA_PIXEL', 'SR_QA_AEROSOL', 'QA_RADSAT', 'ST_B10', 'ST_QA']
# new band names
bns = ['Aerosol','Blue', 'Green', 'Red', 'Nir', 'Swir1', 'Swir2','pixel_qa', 'aerosol_qa', 'radsat_qa', 'SurfaceTemp', 'temp_qa']
...
def RefPull89(image):
  ...
  .combine(ee.Reducer.min().unweighted().forEachBand(pixOut.select(['min_Aerosol','min_Blue', 'min_Green', 'min_Red', 'min_Nir', 'min_Swir1', 'min_Swir2', 'min_SurfaceTemp', 'min_temp_qa'])), sharedInputs = False)
  ...
  .combine(ee.Reducer.mean().unweighted().forEachBand(pixOut.select(['mean_Aerosol','mean_Blue', 'mean_Green', 'mean_Red', 'mean_Nir', 'mean_Swir1', 'mean_Swir2', 'mean_SurfaceTemp', 'mean_temp_qa'])), sharedInputs = False)
  .combine(ee.Reducer.kurtosis().unweighted().forEachBand(pixOut.select(['SurfaceTemp'])), outputPrefix = 'kurt_', sharedInputs = False)
  .combine(ee.Reducer.count().unweighted().forEachBand(pixOut.select(['Aerosol','Blue', 'Green', 'Red', 'Nir', 'Swir1', 'Swir2','dswe', 'dswe3', 'SurfaceTemp'])), outputPrefix = 'pCount_', sharedInputs = False)
...
for tiles in pr:
  ...
  srname = r.proj+'_point_LSC289_'+str(tiles)+'_v'+str(date.today())
  dataOut = (ee.batch.Export.table.toDrive(collection = out,
                                          ...
                                          'kurt_SurfaceTemp', 'prop_clouds','prop_medHighAero','prop_hillShadow','pCount_dswe', 'pCount_dswe3','system:index']))
...
for tiles in pr:
  ...
  # map the refpull function across the 'stack', flatten to an array,
  out = stack.map(RefPull89).flatten()
  srname = r.proj+'_lake_LSC289_'+str(tiles)+'_v'+str(date.today())
  dataOut = (ee.batch.Export.table.toDrive(collection = out,
                                          description = srname,
                                          folder = 'LandsatRepo',
                                          fileFormat = 'csv',
