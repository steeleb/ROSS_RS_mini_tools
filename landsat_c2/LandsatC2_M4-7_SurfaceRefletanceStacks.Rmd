...
def cfMask(image):
  ...
  # qaMask = cloudqa.eq(0)
  return image.addBands(cloudqa)#.addMask(qaMask)
```
...
for tiles in pr:
  ...
  dataOut = (ee.batch.Export.table.toDrive(collection = out,
                                          ...
                                          'prop_clouds','prop_hillShadow','pCount_dswe', 'pCount_dswe3','system:index']))
...
for tiles in pr:
  ...
  # map the refpull function across the 'stack', flatten to an array,
  out = stack.map(RefPull457).flatten()
  srname = r.proj+'_lake_SurfRef457_'+str(tiles)+'_v'+str(date.today())
  dataOut = (ee.batch.Export.table.toDrive(collection = out,
                                          description = srname,
                                          folder = 'LakeReflRepo',
                                          fileFormat = 'csv',
