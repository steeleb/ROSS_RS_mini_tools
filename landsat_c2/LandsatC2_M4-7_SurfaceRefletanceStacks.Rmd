...
def addRadMask(image):
  ...
  satQA = image.select('radsat').eq(0)
  # all must be non-saturated per pixel
  return image.updateMask(satQA)
...
def cfMask(image):
  ...
  #grab just the pixel_qa info
  qa = image.select('pixel_qa')
  cloudqa = (qa.bitwiseAnd(1 << 1).rename('cfmask') #dialated clouds
  ...
  qaMask = cloudqa.eq(0)
  return image.updateMask(qaMask).addBands(cloudqa)
...
def RefPull457(image):
  ...
  dswe3 = d.eq(3).rename('dswe3').selfMask().updateMask(clouds.eq(0)).updateMask(snow.eq(0)).updateMask(s.eq(0))
  ...
  .combine(ee.Reducer.min().unweighted().forEachBand(pixOut.select(['Blue', 'Green', 'Red', 'Nir', 'Swir1', 'Swir2'])), 'min_', False)
  .combine(ee.Reducer.max().unweighted().forEachBand(pixOut.select(['Blue', 'Green', 'Red', 'Nir', 'Swir1', 'Swir2'])), 'max_', False)
  .combine(ee.Reducer.percentile([25]).forEachBand(pixOut.select(['Blue', 'Green', 'Red', 'Nir', 'Swir1', 'Swir2'])), 'Q1_', False)
  .combine(ee.Reducer.percentile([75]).forEachBand(pixOut.select(['Blue', 'Green', 'Red', 'Nir', 'Swir1', 'Swir2'])), 'Q3_', False)
  .combine(ee.Reducer.stdDev().unweighted().forEachBand(pixOut.select(['Blue', 'Green', 'Red', 'Nir', 'Swir1', 'Swir2'])), 'sd_', False)
  .combine(ee.Reducer.count().unweighted().forEachBand(pixOut.select(['dswe3', 'dswe', 'Blue', 'Green', 'Red', 'Nir', 'Swir1', 'Swir2'])), 'pCount_', False)
...
for tiles in pr:
  ...
  dataOut = (ee.batch.Export.table.toDrive(collection = out,
                                          ...
                                          'prop_clouds','prop_hillShadow', 'prop_snow','pCount_dswe1', 'pCount_dswe3','system:index']))
