...
# existing band names
bn457 = ['SR_B1', 'SR_B2', 'SR_B3', 'SR_B4', 'SR_B5', 'SR_B7', 'QA_PIXEL', 'SR_CLOUD_QA', 'QA_RADSAT', 'ST_B6', 'ST_QA']
# new band names
bns = ['Blue', 'Green', 'Red', 'Nir', 'Swir1', 'Swir2', 'pixel_qa', 'cloud_qa', 'radsat_qa', 'SurfaceTemp', 'temp_qa']
...
def RefPull457(image):
  ...
  .combine(ee.Reducer.min().unweighted().forEachBand(pixOut.select(['min_Blue', 'min_Green', 'min_Red', 'min_Nir', 'min_Swir1', 'min_Swir2', 'min_SurfaceTemp', 'min_temp_qa'])), sharedInputs = False)
  ...
  .combine(ee.Reducer.mean().unweighted().forEachBand(pixOut.select(['mean_Blue', 'mean_Green', 'mean_Red', 'mean_Nir', 'mean_Swir1', 'mean_Swir2', 'mean_SurfaceTemp', 'mean_temp_qa'])), sharedInputs = False)
  .combine(ee.Reducer.count().unweighted().forEachBand(pixOut.select(['Blue', 'Green', 'Red', 'Nir', 'Swir1', 'Swir2','dswe', 'dswe3', 'SurfaceTemp'])), outputPrefix = 'pCount_', sharedInputs = False)
...
for tiles in pr:
  ...
  srname = r.proj+'_point_LSC2457_'+str(tiles)+'_v'+str(date.today())
  dataOut = (ee.batch.Export.table.toDrive(collection = out,
                                          ...
                                          'Q3_Blue', 'Q3_Green', 'Q3_Red', 'Q3_Nir', 'Q3_Swir1', 'Q3_Swir2', 'Q3_SurfaceTemp', 
                                          'sd_Blue', 'sd_Green', 'sd_Red', 'sd_Nir', 'sd_Swir1', 'sd_Swir2', 'sd_SurfaceTemp', 'sd_temp_qa',
                                          'pCount_Blue', 'pCount_Green', 'pCount_Red', 'pCount_Nir', 'pCount_Swir1', 'pCount_Swir2', 'pCount_SurfaceTemp',
                                          'kurt_SurfaceTemp','prop_clouds','prop_hillShadow','pCount_dswe', 'pCount_dswe3','system:index']))
...
for tiles in pr:
  ...
  # map the refpull function across the 'stack', flatten to an array,
  out = stack.map(RefPull457).flatten()
  srname = r.proj+'_lake_LSC2457_'+str(tiles)+'_v'+str(date.today())
  dataOut = (ee.batch.Export.table.toDrive(collection = out,
                                          description = srname,
                                          folder = 'LandsatRepo',
                                          fileFormat = 'csv',
