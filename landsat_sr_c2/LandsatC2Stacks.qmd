...
def dpBuff(i):
  return i.buffer(120) #doing a 50m buffer for this project
...
def AddTmask(image):
  ...
  #only pass through values less than 2 deg error
  tmask = (tqa.lte(2).rename('tmask')
    .updateMask(tqa.gte(0)))
...
```{python}
def TempPull(image):
  # process image with tmask, and grab tmask band only
  f = AddTmask(image).select('tmask')
  # where the f mask is 1 (valid temp), call that 1 (otherwise 0) and rename as validTemp.
  validTemp = f.eq(1).rename('validTemp')
  d = Dswe(image).select('dswe')
  newImage = image.select('SurfTemp')
  pixOut = (newImage.addBands(validTemp)
            .addBands(d)
            .updateMask(d.eq(1)))
  combinedReducer = ((ee.Reducer.stdDev().unweighted().forEachBand(pixOut.select('SurfTemp').rename('sd_SurfTemp')))
    .combine(ee.Reducer.median().unweighted().forEachBand(pixOut.select('SurfTemp')), 'med_', False)
...
for tiles in pr:
  ...
  dataOut = ee.batch.Export.table.toDrive(collection = out,\
                                          ...
                                          folder = 'LakeReflRepo',\
                                          fileFormat = 'csv',\
                                          selectors = ['Aerosol','Blue', 'Green', 'Red', 'Nir', 'Swir1', 'Swir2','pixel_qa', 'hillShadow', 'sd_NirSD','cScore_clouds','pCount_dswe3','pCount_dswe1','system:index','system:feature'])
...
for tiles in pr:
  ...
  tempDataOut = ee.batch.Export.table.toDrive(collection = tempOut,\
                                          ...
                                          folder = 'LakeTempRepo',\
                                          fileFormat = 'csv',\
                                          selectors = ['med_SurfTemp','sd_SurfTemp', 'pCount_SurfTemp', 'system:index'])
  #Check how many existing tasks are running and take a break of 120 secs if it's >25 
  maximum_no_of_tasks(25, 120)
  #Send next task.                                        
  tempDataOut.start()
