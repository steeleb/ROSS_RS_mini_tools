...
locs_file = 'sites/upper_poudre_lakes_v2.csv'
...
def AddTMask(image):
  ...
  tmask = (water.gt(0).rename(['tmask']) #FIND WATER!
    .where(snow.gt(0), ee.Image(0)) #where there's snow give it a value of 0
    .where(cloudshadow.gt(0), ee.Image(0)) # and cloud shadows
    .where(cloud.gt(0), ee.Image(0)) #and clouds
    .updateMask(water.gt(0))) #mask the fmask so that it has the same footprint as the quality (BQA) band
  #add mask to include only pixels with error less than 1deg C
  tmask = tmask.updateMask(tqa.lte(1))
...
for tiles in pr:
  ...
  tempDataOut = ee.batch.Export.table.toDrive(collection = tempOut,\
                                          ...
                                          folder = 'LakeTempRepo',\
                                          fileFormat = 'csv',\
                                          selectors = ['SurfTemp','sd_SurfTemp', 'min_SurfTempMin', 'max_SurfTempMax', 'pCount_tmask', 'system:index'])
  #Check how many existing tasks are running and take a break of 120 secs if it's >25 
  maximum_no_of_tasks(25, 120)
  #Send next task.                                        
  tempDataOut.start()
