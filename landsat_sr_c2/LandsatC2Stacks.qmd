...
def AddTmask(image):
  ...
  #only pass through values less than 3 deg error
  tmask = (tqa.lte(1).rename('tmask')
...
def TempPull(image):
  ...
  combinedReducer = ((ee.Reducer.median().unweighted().forEachBand(pixOut.select('SurfTemp')))
    .combine(ee.Reducer.stdDev().unweighted().forEachBand(pixOut.select('SurfTemp')), 'sd_', False)
...
for tiles in pr:
  ...
  srname = 'SurfRef_'+str(tiles)+'_v'+str(date.today())
...
for tiles in pr:
  ...
  tempOut = stack.map(TempPull)\
    .flatten()
  stname = 'SurfTemp_'+str(tiles)+'_v'+str(date.today())
  tempDataOut = ee.batch.Export.table.toDrive(collection = tempOut,\
                                          description = stname,\
                                          folder = 'LakeTempRepo',\
                                          fileFormat = 'csv',\
                                          selectors = ['SurfTemp','sd_SurfTemp', 'pCount_SurfTemp', 'system:index'])
  #Check how many existing tasks are running and take a break of 120 secs if it's >25 
  maximum_no_of_tasks(25, 120)
  #Send next task.                                        
  tempDataOut.start()
