...
for tiles in pr:
    ...
    lakes = dp.filterBounds(tile.geometry())\
      .map(dpBuff)
    # snip the ls data by the geometry of the 'lakes'    
    stack = ls.filterBounds(lakes.geometry())
    # map the refpull function across the 'stack', flatten to an array, filter to remove scenes with clouds less than 50%.
    ...
    dataOut = ee.batch.Export.table.toDrive(collection = out,\
                                            ...
                                            folder = 'LakeReflRepo',\
                                            fileFormat = 'csv',\
                                            selectors = ['Aerosol','Blue', 'Green', 'Red', 'Nir', 'Swir1', 'Swir2', 'SurfTemp','pixel_qa', 'hillShadow', 'sd_NirSD','cScore_clouds','pCount_dswe3','pCount_dswe1','system:index','permanent','distance'])
